<CRITICAL_READ_FIRST>
  HARD RULE: never change a file until the user has explicitly approved the plan. Investigate
  read-only, explain the problem and the proposed fix in plain language, wait for a yes, then
  build. See root CLAUDE.md <APPROVAL_REQUIRED_BEFORE_ANY_CHANGE>.

  ONE SOURCE OF WATER, MANY FAUCETS. Mobile and web are two faucets on one source — turn on
  either and the same water comes out, not the same 20 ounces. What must match is FUNCTIONALITY:
  same features, rules, permissions, endpoints, data shapes, and user-visible outcome. The CODE
  should differ where the platforms differ — use web idiom (div/Tailwind, React Router,
  localStorage, hover, responsive desktop layouts), not a pixel-copy of React Native.

  EVERY prompt gets checked against `webapp/mobile/src/` FIRST — grep/read the matching screen,
  store or hook before writing code or proposing a plan. Then:
  - Exists on mobile only → bring the behaviour here, in web idiom.
  - Exists on neither → say so, build it on both.
  - A capability or rule one side has and the other lacks is a BUG. Flag it before shipping.
  - Instructions given about web ALSO apply to mobile, and vice versa — deliver both together.
  - Never add a backend route for the web app; use the endpoint mobile already calls.
  - State which files you checked on each side.

  Never build from imagination, never invent data shapes, never write summary/audit markdown.
  See root CLAUDE.md <FEATURE_PARITY_BOTH_FAUCETS> and <CRITICAL_READ_FIRST> for the full rule.
  NOTE: the <bootstrapping> and mock-data guidance below does NOT apply — this is not a fresh
  workspace and mock data is forbidden; use the real backend the mobile app uses.
</CRITICAL_READ_FIRST>
