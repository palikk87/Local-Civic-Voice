 ...
 *
 * There is no browser-side brief writer any more. If no official source yields
 * text, the server reports `unavailable` and we say so — we never invent a brief.
 */
import { useQuery } from "@tanstack/react-query";
import { civicApi, type CitizenBriefLabels, type CitizenBriefSections } from "@/lib/civic";
