  ...
  persist(
    (set, get) => ({
      ...
      voteOnReference: (referenceId, vote, sourcePostId, sourceAuthorId) => {
        ...
        set((state) => {
          ...
          if (contributorIdx >= 0) {
            ...
            // Find author info
            // The id is all we have here; the display fields are filled in by
            // whatever renders this from real profile data. It used to look the
            // author up in sampleUsers and fall back to a stock photograph.
            const author = {
              username: 'user',
              displayName: 'User',
      ...
      recalculateEngagementLeaders: () => {
        ...
        // An empty leaderboard is allowed to be empty. This used to fall back
