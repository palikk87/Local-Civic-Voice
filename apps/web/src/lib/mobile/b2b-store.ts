// Web port of mobile/src/lib/b2b-store.ts — zustand persist uses localStorage
// instead of AsyncStorage; same /api/b2b endpoints and session shape.
...
export const useB2BStore = create<B2BState>()(
  persist(
    (set, get) => ({
      ...
      isLoading: false,
      // Unlike mobile's AsyncStorage, localStorage rehydration is synchronous —
      // the store is already hydrated by the time anything renders. (The mobile
      // onRehydrateStorage pattern would fire before this const exists and the
      // flag would never flip, leaving the portal on an endless spinner.)
      _hasHydrated: true,
...
/** Authorization header for /api/b2b/* calls, or {} when not signed in. */
export function b2bAuthHeader(): Record<string, string> {
  const token = useB2BStore.getState().session?.token;
