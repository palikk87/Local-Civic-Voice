...
export const selectCivicScore = (state: GamificationState | undefined) =>
  state?.civicScore ?? { total: 0, level: 'newcomer', xpToNextLevel: 100 };
export const selectStreak = (state: GamificationState | undefined) =>
  state?.streak ?? { current: 0, lastActivityDate: '', freezeDeadline: null };
