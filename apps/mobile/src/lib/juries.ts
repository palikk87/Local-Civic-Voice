 ...
 *
 * PHONE TWIN of apps/web/src/lib/juries.ts. Kept by hand and deliberately
 * identical below the import line: the two clients read the same routes, and a
 * shape that drifted on one platform would be a juror who could not see the
 * case on their phone.
 *
 * NOTHING HERE NAMES A JUROR. The server never sends one: a seat is a state and
 * a timestamp, and only the viewer's own seat is marked.
 */
