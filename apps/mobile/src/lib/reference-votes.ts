 ...
 *      authoritative weighted tally (delegations included),
 ...
 *
 * Web twin: webapp/src/lib/mobile/reference-votes.ts
 */
...
/**
 * Mirror a vote the server told us about (e.g. a detail screen loading
...
export async function castReferenceVote(
  ...
  // here, so the question is here rather than on each screen's buttons — see
  ...
  try {
    ...
    // The mobile api helper surfaces the server's error message, not the HTTP
    // status — "Reference not found" is the vote endpoint's 404.
