...
export const useTimelineStore = create<TimelineState>()(
  persist(
    (set, get) => ({
      ...
      isLoading: false,
      feedError: null,
      // Empty. This used to manufacture direct-message threads with invented
      // people at store creation, on every launch.
      conversations: [],
      ...
      likeComment: (postId, commentId) => {
        ...
        void api.post(`/api/posts/${postId}/comments/${commentId}/like`).catch(() => {
      ...
      loadMessages: (conversationId) => {
        ...
        // Nothing to load here. Real direct messages come from
      ...
      shareToMessage: (userId, post) => {
        ...
        // /api/users/discover; carry the id through.
      ...
      searchUsers: (query) => {
        // Mention autocomplete used to search the mock cast, so typing "@"
        // offered people who do not exist. Real search is /api/users/search;
        // until a caller wires it in, offer nobody rather than fiction.
