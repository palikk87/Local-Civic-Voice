 ...
 *
 * Safe to re-run. On an account that already exists it updates the display
 * name, tier, password AND api key. So this is also how a B2B password or key
 * is rotated: change the variable, run it again.
 *
...
async function upsertClient(account: SeedAccount): Promise<void> {
  ...
  if (existing) {
    ...
    console.log("  Password and API key rotated.");
    return;
  }
